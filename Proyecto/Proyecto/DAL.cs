...
namespace Proyecto
{
    public class DAL
    {
        ...
        public int agregarUsuario(int dni, string nombre, string apellido, string mail, string clave, int intentosFallidos, bool bloqueado, bool esAdmin, double credito)
        {
            int resultadoQuery;
            int idNuevoUsuario = -1;
            string queryString = 
                "INSERT INTO [dbo].[usuario] ([dni], [nombre], [apellido], [mail], [contraseña], [intentosFallidos], [bloqueado], [esAdmin], [credito]) " +
                "VALUES (@dni, @nombre, @apellido, @mail, @clave, @intentosFallidos, @bloqueado, @esAdmin, @credito);";
            using (SqlConnection connection =
                new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
        ...
        public int agregarVuelo(int origen_fk, int destino_fk, int capacidad, int vendido, double costo, DateTime fecha, string aerolinea, string avion)
        {
            int resultadoQuery;
            int idNuevoVuelo = -1;
            string queryString =
                "INSERT INTO [dbo].[vuelo] ([capacidad], [vendido], [costo], [fecha], [aerolinea], [avion], [origen_fk], [destino_fk]) " +
                "VALUES (@capacidad, @vendido, @costo, @fecha, @aerolinea, @avion, @origen_fk, @destino_fk);";
            using (SqlConnection connection =
                new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
                command.Parameters.Add(new SqlParameter("@capacidad", SqlDbType.Int));
                command.Parameters.Add(new SqlParameter("@vendido", SqlDbType.Int));
                command.Parameters.Add(new SqlParameter("@costo", SqlDbType.Decimal));
                ...
                command.Parameters["@capacidad"].Value = capacidad;
                command.Parameters["@vendido"].Value = vendido;
                command.Parameters["@costo"].Value = costo;
        ...
        {
            string queryString =
                "INSERT INTO [dbo].[usuario_hotel] ([usuario_fk], [hotel_fk], [cantidad]) " +
                "VALUES (@usuario_fk, @hotel_fk, @cantidad);";
            using (SqlConnection connection =
                new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
        ...
        public int modificarUsuarioHotel(int usuario_fk, int hotel_fk, int cantidad)
        {
            string querySrting =
                "UPDATE [dbo].[usuario_hotel] " +
                "SET cantidad += @cantidad " +
                "WHERE usuario_fk = @usuario_fk " +
                "AND hotel_fk = @hotel_fk;";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(querySrting, connection);
                command.Parameters.Add(new SqlParameter("@usuario_fk", SqlDbType.Int));
        ...
        public int eliminarUsuarioHotel(int usuario_fk, int hotel_fk)
        {
            string queryString = "DELETE FROM [dbo].[usuario_hotel] " +
                "WHERE usuario_fk = @usuario_fk " +
                "AND hotel_fk = @hotel_fk;";
            using (SqlConnection connection =
                new SqlConnection(connectionString))
        ...
        public int eliminarUsuarioVuelo(int usuario_fk, int vuelo_fk)
        {
            string queryString = "DELETE FROM [dbo].[usuario_vuelo] " +
                "WHERE usuario_fk = @usuario_fk " +
