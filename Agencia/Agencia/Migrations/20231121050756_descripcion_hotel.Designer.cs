// <auto-generated />
using System;
using Agencia.Models;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Agencia.Migrations
{
    [DbContext(typeof(Context))]
    [Migration("20231121050756_descripcion_hotel")]
    partial class descripcion_hotel
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.23")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("Agencia.Models.Ciudad", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"), 1L, 1);

                    b.Property<string>("nombre")
                        .IsRequired()
                        .HasColumnType("varchar(50)");

                    b.HasKey("id");

                    b.ToTable("ciudad", (string)null);

                    b.HasData(
                        new
                        {
                            id = 1,
                            nombre = "Buenos Aires"
                        },
                        new
                        {
                            id = 2,
                            nombre = "Córdoba"
                        },
                        new
                        {
                            id = 3,
                            nombre = "Rosario"
                        },
                        new
                        {
                            id = 4,
                            nombre = "Mendoza"
                        },
                        new
                        {
                            id = 5,
                            nombre = "La Plata"
                        },
                        new
                        {
                            id = 6,
                            nombre = "Mar del Plata"
                        },
                        new
                        {
                            id = 7,
                            nombre = "San Miguel de Tucumán"
                        },
                        new
                        {
                            id = 8,
                            nombre = "Salta"
                        },
                        new
                        {
                            id = 9,
                            nombre = "Santa Fe"
                        },
                        new
                        {
                            id = 10,
                            nombre = "San Juan"
                        },
                        new
                        {
                            id = 11,
                            nombre = "Resistencia"
                        },
                        new
                        {
                            id = 12,
                            nombre = "Corrientes"
                        },
                        new
                        {
                            id = 13,
                            nombre = "Posadas"
                        },
                        new
                        {
                            id = 14,
                            nombre = "Neuquén"
                        },
                        new
                        {
                            id = 15,
                            nombre = "Formosa"
                        },
                        new
                        {
                            id = 16,
                            nombre = "Bahía Blanca"
                        },
                        new
                        {
                            id = 17,
                            nombre = "San Salvador de Jujuy"
                        },
                        new
                        {
                            id = 18,
                            nombre = "Río Cuarto"
                        },
                        new
                        {
                            id = 19,
                            nombre = "Comodoro Rivadavia"
                        },
                        new
                        {
                            id = 20,
                            nombre = "Concordia"
                        });
                });

            modelBuilder.Entity("Agencia.Models.Habitacion", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"), 1L, 1);

                    b.Property<int>("capacidad")
                        .HasColumnType("int");

                    b.Property<double>("costo")
                        .HasColumnType("float");

                    b.Property<int>("hotel_fk")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.HasIndex("hotel_fk");

                    b.ToTable("habitacion", (string)null);

                    b.HasData(
                        new
                        {
                            id = 1,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 1
                        },
                        new
                        {
                            id = 2,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 1
                        },
                        new
                        {
                            id = 3,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 1
                        },
                        new
                        {
                            id = 4,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 1
                        },
                        new
                        {
                            id = 5,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 1
                        },
                        new
                        {
                            id = 6,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 1
                        },
                        new
                        {
                            id = 7,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 2
                        },
                        new
                        {
                            id = 8,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 2
                        },
                        new
                        {
                            id = 9,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 2
                        },
                        new
                        {
                            id = 10,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 2
                        },
                        new
                        {
                            id = 11,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 2
                        },
                        new
                        {
                            id = 12,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 2
                        },
                        new
                        {
                            id = 13,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 3
                        },
                        new
                        {
                            id = 14,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 3
                        },
                        new
                        {
                            id = 15,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 3
                        },
                        new
                        {
                            id = 16,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 3
                        },
                        new
                        {
                            id = 17,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 3
                        },
                        new
                        {
                            id = 18,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 3
                        },
                        new
                        {
                            id = 19,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 4
                        },
                        new
                        {
                            id = 20,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 4
                        },
                        new
                        {
                            id = 21,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 4
                        },
                        new
                        {
                            id = 22,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 4
                        },
                        new
                        {
                            id = 23,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 4
                        },
                        new
                        {
                            id = 24,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 4
                        },
                        new
                        {
                            id = 25,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 5
                        },
                        new
                        {
                            id = 26,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 5
                        },
                        new
                        {
                            id = 27,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 5
                        },
                        new
                        {
                            id = 28,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 5
                        },
                        new
                        {
                            id = 29,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 5
                        },
                        new
                        {
                            id = 30,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 5
                        },
                        new
                        {
                            id = 31,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 6
                        },
                        new
                        {
                            id = 32,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 6
                        },
                        new
                        {
                            id = 33,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 6
                        },
                        new
                        {
                            id = 34,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 6
                        },
                        new
                        {
                            id = 35,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 6
                        },
                        new
                        {
                            id = 36,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 6
                        },
                        new
                        {
                            id = 37,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 7
                        },
                        new
                        {
                            id = 38,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 7
                        },
                        new
                        {
                            id = 39,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 7
                        },
                        new
                        {
                            id = 40,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 7
                        },
                        new
                        {
                            id = 41,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 7
                        },
                        new
                        {
                            id = 42,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 7
                        },
                        new
                        {
                            id = 43,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 8
                        },
                        new
                        {
                            id = 44,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 8
                        },
                        new
                        {
                            id = 45,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 8
                        },
                        new
                        {
                            id = 46,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 8
                        },
                        new
                        {
                            id = 47,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 8
                        },
                        new
                        {
                            id = 48,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 8
                        },
                        new
                        {
                            id = 49,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 9
                        },
                        new
                        {
                            id = 50,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 9
                        },
                        new
                        {
                            id = 51,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 9
                        },
                        new
                        {
                            id = 52,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 9
                        },
                        new
                        {
                            id = 53,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 9
                        },
                        new
                        {
                            id = 54,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 9
                        },
                        new
                        {
                            id = 55,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 10
                        },
                        new
                        {
                            id = 56,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 10
                        },
                        new
                        {
                            id = 57,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 10
                        },
                        new
                        {
                            id = 58,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 10
                        },
                        new
                        {
                            id = 59,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 10
                        },
                        new
                        {
                            id = 60,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 10
                        },
                        new
                        {
                            id = 61,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 11
                        },
                        new
                        {
                            id = 62,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 11
                        },
                        new
                        {
                            id = 63,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 11
                        },
                        new
                        {
                            id = 64,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 11
                        },
                        new
                        {
                            id = 65,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 11
                        },
                        new
                        {
                            id = 66,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 11
                        },
                        new
                        {
                            id = 67,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 12
                        },
                        new
                        {
                            id = 68,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 12
                        },
                        new
                        {
                            id = 69,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 12
                        },
                        new
                        {
                            id = 70,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 12
                        },
                        new
                        {
                            id = 71,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 12
                        },
                        new
                        {
                            id = 72,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 12
                        },
                        new
                        {
                            id = 73,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 13
                        },
                        new
                        {
                            id = 74,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 13
                        },
                        new
                        {
                            id = 75,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 13
                        },
                        new
                        {
                            id = 76,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 13
                        },
                        new
                        {
                            id = 77,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 13
                        },
                        new
                        {
                            id = 78,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 13
                        },
                        new
                        {
                            id = 79,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 14
                        },
                        new
                        {
                            id = 80,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 14
                        },
                        new
                        {
                            id = 81,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 14
                        },
                        new
                        {
                            id = 82,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 14
                        },
                        new
                        {
                            id = 83,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 14
                        },
                        new
                        {
                            id = 84,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 14
                        },
                        new
                        {
                            id = 85,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 15
                        },
                        new
                        {
                            id = 86,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 15
                        },
                        new
                        {
                            id = 87,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 15
                        },
                        new
                        {
                            id = 88,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 15
                        },
                        new
                        {
                            id = 89,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 15
                        },
                        new
                        {
                            id = 90,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 15
                        },
                        new
                        {
                            id = 91,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 16
                        },
                        new
                        {
                            id = 92,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 16
                        },
                        new
                        {
                            id = 93,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 16
                        },
                        new
                        {
                            id = 94,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 16
                        },
                        new
                        {
                            id = 95,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 16
                        },
                        new
                        {
                            id = 96,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 16
                        },
                        new
                        {
                            id = 97,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 17
                        },
                        new
                        {
                            id = 98,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 17
                        },
                        new
                        {
                            id = 99,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 17
                        },
                        new
                        {
                            id = 100,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 17
                        },
                        new
                        {
                            id = 101,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 17
                        },
                        new
                        {
                            id = 102,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 17
                        },
                        new
                        {
                            id = 103,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 18
                        },
                        new
                        {
                            id = 104,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 18
                        },
                        new
                        {
                            id = 105,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 18
                        },
                        new
                        {
                            id = 106,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 18
                        },
                        new
                        {
                            id = 107,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 18
                        },
                        new
                        {
                            id = 108,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 18
                        },
                        new
                        {
                            id = 109,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 19
                        },
                        new
                        {
                            id = 110,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 19
                        },
                        new
                        {
                            id = 111,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 19
                        },
                        new
                        {
                            id = 112,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 19
                        },
                        new
                        {
                            id = 113,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 19
                        },
                        new
                        {
                            id = 114,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 19
                        },
                        new
                        {
                            id = 115,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 20
                        },
                        new
                        {
                            id = 116,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 20
                        },
                        new
                        {
                            id = 117,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 20
                        },
                        new
                        {
                            id = 118,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 20
                        },
                        new
                        {
                            id = 119,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 20
                        },
                        new
                        {
                            id = 120,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 20
                        },
                        new
                        {
                            id = 121,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 2021
                        },
                        new
                        {
                            id = 122,
                            capacidad = 2,
                            costo = 20000.0,
                            hotel_fk = 2021
                        },
                        new
                        {
                            id = 123,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 2021
                        },
                        new
                        {
                            id = 124,
                            capacidad = 4,
                            costo = 35000.0,
                            hotel_fk = 2021
                        },
                        new
                        {
                            id = 125,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 2021
                        },
                        new
                        {
                            id = 126,
                            capacidad = 8,
                            costo = 70000.0,
                            hotel_fk = 2021
                        });
                });

            modelBuilder.Entity("Agencia.Models.Hotel", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"), 1L, 1);

                    b.Property<int>("ciudad_fk")
                        .HasColumnType("int");

                    b.Property<string>("descripcion")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("imagen")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("nombre")
                        .IsRequired()
                        .HasColumnType("varchar(50)");

                    b.HasKey("id");

                    b.HasIndex("ciudad_fk");

                    b.ToTable("hotel", (string)null);

                    b.HasData(
                        new
                        {
                            id = 1,
                            ciudad_fk = 7,
                            descripcion = "San Miguel de Tucumán, Centro histórico. A 287 m del centro",
                            imagen = "images/hotel/04a270ea-1772-4970-84ea-8511a77d344b_hotelGardenPlaza-Tucuman.png",
                            nombre = "Garden Plaza"
                        },
                        new
                        {
                            id = 2,
                            ciudad_fk = 2,
                            descripcion = "Córdoba, Barrio ItuzaingóA 445 m del centro",
                            imagen = "images/hotel/04314e9f-5c9a-4f8a-8672-94781301b98f_selinaNuevaCordoba-Cordoba.png",
                            nombre = "Selina Nueva Cordoba"
                        },
                        new
                        {
                            id = 3,
                            ciudad_fk = 3,
                            descripcion = "Rosario, Centro. A 966 m del centro",
                            imagen = "images/hotel/79fdae4f-dfee-4262-9803-b1094618f216_americanPuertoRosarioHotel-Rosario.png",
                            nombre = "American Puerto Rosario Hotel"
                        },
                        new
                        {
                            id = 4,
                            ciudad_fk = 4,
                            descripcion = "Mendoza, Centro. A 390 m del centro",
                            imagen = "images/hotel/c9252ae1-4112-4f7e-9ca2-7b87cb681dda_raicesAconcaguaHotel-Mendoza.png",
                            nombre = "Raices Aconcagua Hotel"
                        },
                        new
                        {
                            id = 5,
                            ciudad_fk = 5,
                            descripcion = "Buenos Aires, La Plata. A 2,5 km del Museo de la Plata",
                            imagen = "images/hotel/0bf11e5f-3554-42a0-b0e0-6087fd485599_dazzlerByWyndhamLaPlata-La Plata.png",
                            nombre = "Dazzler by Wyndham La Plata"
                        },
                        new
                        {
                            id = 6,
                            ciudad_fk = 6,
                            descripcion = "Mar del Plata. A 9,13 km del centro",
                            imagen = "images/hotel/5b90c67e-11a4-45f7-aa5f-36101400d3ac_hotelAatracMarDelPlata-Mar del Plata.png",
                            nombre = "Hotel Aatrac Mar del Plata"
                        },
                        new
                        {
                            id = 7,
                            ciudad_fk = 7,
                            descripcion = " San Miguel de Tucumán, Argentina. A 3,39 km del centro",
                            imagen = "images/hotel/bdbb374f-432c-4147-a5ce-499bbc0a292f_hotelPortalDelNorte-Tucuman.png",
                            nombre = "Hotel Portal del Norte"
                        },
                        new
                        {
                            id = 8,
                            ciudad_fk = 8,
                            descripcion = "Salta, Centro. A 226 m del centro",
                            imagen = "images/hotel/a917a5f2-964c-4bb7-94d1-ec4ba0c2baae_hotelPosadaDelSolSalta-Salta.png",
                            nombre = "Hotel Posada del Sol Salta"
                        },
                        new
                        {
                            id = 9,
                            ciudad_fk = 9,
                            descripcion = "Santa Fe, Argentina. A 1,93 km del centro",
                            imagen = "images/hotel/3bd2a198-62ac-47cd-8158-fd910db6fb15_hotelCastelar-Santa Fe.png",
                            nombre = "Hotel Castelar"
                        },
                        new
                        {
                            id = 10,
                            ciudad_fk = 10,
                            descripcion = "San Juan. A 205 m del centro",
                            imagen = "images/hotel/779f2715-5640-4c7e-af86-dbfc2f2b05a4_alhambraInnHotel-San Juan.png",
                            nombre = "Alhambra Inn Hotel"
                        },
                        new
                        {
                            id = 11,
                            ciudad_fk = 1,
                            descripcion = "Buenos Aires, Recoleta. A 1,10 km del centro",
                            imagen = "images/hotel/97148535-98f8-41a2-8584-678d3874d480_granHotelBuenosAires-Buenos Aires.png",
                            nombre = "Gran Hotel Buenos Aires"
                        },
                        new
                        {
                            id = 12,
                            ciudad_fk = 2,
                            descripcion = "Córdoba, Argentina. A 31,70 km del centro",
                            imagen = "images/hotel/00475231-6fa4-4b8b-9ee6-5837912b6f28_howardJohnsonByWyndhamCordoba-Cordoba.png",
                            nombre = "Howard Johnson by Wyndham Cordoba"
                        },
                        new
                        {
                            id = 13,
                            ciudad_fk = 3,
                            descripcion = "Rosario, Argentina. A 2,38 km del centro",
                            imagen = "images/hotel/27806441-6c89-466a-9766-3264bb46cdaf_aristonHotel-Rosario.png",
                            nombre = "Ariston Hotel"
                        },
                        new
                        {
                            id = 14,
                            ciudad_fk = 4,
                            descripcion = "Mendoza, Argentina. A 346 m del centro",
                            imagen = "images/hotel/8477278d-e8cc-49fe-8afb-1182f09f77e0_hotelPrincessGold-Mendoza.png",
                            nombre = "Hotel Princess Gold"
                        },
                        new
                        {
                            id = 15,
                            ciudad_fk = 5,
                            descripcion = "Buenos Aires, La Plata. A 5,35 km del centro",
                            imagen = "images/hotel/6c8fbd02-09ed-49c2-b4cc-3e2bbb0c17f9_hotelDelSol-La Plata.png",
                            nombre = "Hotel del Sol"
                        },
                        new
                        {
                            id = 16,
                            ciudad_fk = 6,
                            descripcion = "Buenos Aires, Argentina. A 297 m del centro",
                            imagen = "images/hotel/d76a8e6a-4453-4e94-9cf5-2f09fe59a298_hotelArgentino-Mar del Plata.png",
                            nombre = "Hotel Argentino"
                        },
                        new
                        {
                            id = 17,
                            ciudad_fk = 7,
                            descripcion = "San Miguel de Tucumán, Argentina. A 1,81 km del centro",
                            imagen = "images/hotel/dd74e84b-2fa4-455f-9064-a20e5cbb8230_hotelLePark-tucuman.png",
                            nombre = "Hotel Le Park"
                        },
                        new
                        {
                            id = 18,
                            ciudad_fk = 8,
                            descripcion = "Salta, Centro. A 704 m del centro",
                            imagen = "images/hotel/6e456053-c5b0-462e-a510-2722dbe875b6_hotelSamka-Salta.png",
                            nombre = "Hotel Samka"
                        },
                        new
                        {
                            id = 19,
                            ciudad_fk = 9,
                            descripcion = "Santa Fe. A 2,37 km del centro",
                            imagen = "images/hotel/15c46820-eb41-468f-833e-8a1aae28eade_puertoAmarrasHotel&Suites-Santa Fe.png",
                            nombre = "Puerto Amarras Hotel & Suites"
                        },
                        new
                        {
                            id = 20,
                            ciudad_fk = 10,
                            descripcion = "San Juan, Argentina. A 43 m del centro",
                            imagen = "images/hotel/7ccd44f5-2b8d-4217-b152-10508cd991e0_hotelAlbertina-San Juan.png",
                            nombre = "Hotel Albertina"
                        },
                        new
                        {
                            id = 2021,
                            ciudad_fk = 7,
                            descripcion = "San Miguel de Tucumán, Barrio Norte. A 1,34 km del centro",
                            imagen = "images/hotel/9d04ad05-788c-4f24-b594-f7944b62eb62_sheratonTucumanHotel-Tucuman.png",
                            nombre = "Sheraton Tucuman Hotel"
                        });
                });

            modelBuilder.Entity("Agencia.Models.ReservaHabitacion", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"), 1L, 1);

                    b.Property<int>("cantPersonas")
                        .HasColumnType("int");

                    b.Property<DateTime>("fechaDesde")
                        .HasColumnType("datetime");

                    b.Property<DateTime>("fechaHasta")
                        .HasColumnType("datetime");

                    b.Property<int>("habitacion_fk")
                        .HasColumnType("int");

                    b.Property<double>("pagado")
                        .HasColumnType("float");

                    b.Property<int>("usuarioRH_fk")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.HasIndex("habitacion_fk");

                    b.HasIndex("usuarioRH_fk");

                    b.ToTable("reservaHotel", (string)null);

                    b.HasData(
                        new
                        {
                            id = 1,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 10, 1, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 10, 5, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 1,
                            pagado = 150000.0,
                            usuarioRH_fk = 1
                        },
                        new
                        {
                            id = 2,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 11, 15, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 11, 20, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 7,
                            pagado = 120000.0,
                            usuarioRH_fk = 2
                        },
                        new
                        {
                            id = 3,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 10, 3, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 11, 3, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 13,
                            pagado = 170000.0,
                            usuarioRH_fk = 3
                        },
                        new
                        {
                            id = 4,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 9, 20, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 9, 25, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 19,
                            pagado = 140000.0,
                            usuarioRH_fk = 4
                        },
                        new
                        {
                            id = 5,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 11, 1, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 11, 10, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 25,
                            pagado = 110000.0,
                            usuarioRH_fk = 5
                        },
                        new
                        {
                            id = 6,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 10, 8, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 10, 12, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 1,
                            pagado = 150000.0,
                            usuarioRH_fk = 6
                        },
                        new
                        {
                            id = 7,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 12, 5, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 12, 10, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 7,
                            pagado = 120000.0,
                            usuarioRH_fk = 7
                        },
                        new
                        {
                            id = 8,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 9, 25, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 9, 30, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 13,
                            pagado = 170000.0,
                            usuarioRH_fk = 8
                        },
                        new
                        {
                            id = 9,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 11, 12, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 11, 17, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 19,
                            pagado = 140000.0,
                            usuarioRH_fk = 9
                        },
                        new
                        {
                            id = 10,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 12, 20, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 12, 25, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 25,
                            pagado = 110000.0,
                            usuarioRH_fk = 10
                        },
                        new
                        {
                            id = 1002,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 10, 20, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 10, 24, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 1,
                            pagado = 150000.0,
                            usuarioRH_fk = 3
                        },
                        new
                        {
                            id = 2007,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 10, 25, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 10, 31, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 7,
                            pagado = 120000.0,
                            usuarioRH_fk = 1024
                        },
                        new
                        {
                            id = 2008,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 10, 11, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 10, 18, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 25,
                            pagado = 110000.0,
                            usuarioRH_fk = 1024
                        },
                        new
                        {
                            id = 2009,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 10, 25, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 10, 25, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 31,
                            pagado = 160000.0,
                            usuarioRH_fk = 1024
                        },
                        new
                        {
                            id = 2010,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 10, 23, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 10, 23, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 37,
                            pagado = 135000.0,
                            usuarioRH_fk = 1024
                        },
                        new
                        {
                            id = 2011,
                            cantPersonas = 2,
                            fechaDesde = new DateTime(2023, 10, 2, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            fechaHasta = new DateTime(2023, 10, 5, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            habitacion_fk = 25,
                            pagado = 110000.0,
                            usuarioRH_fk = 1024
                        });
                });

            modelBuilder.Entity("Agencia.Models.ReservaVuelo", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"), 1L, 1);

                    b.Property<int>("cantPersonas")
                        .HasColumnType("int");

                    b.Property<double>("pagado")
                        .HasColumnType("float");

                    b.Property<int>("usuarioRV_fk")
                        .HasColumnType("int");

                    b.Property<int>("vuelo_fk")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.HasIndex("usuarioRV_fk");

                    b.HasIndex("vuelo_fk");

                    b.ToTable("reservaVuelo", (string)null);

                    b.HasData(
                        new
                        {
                            id = 1,
                            cantPersonas = 1,
                            pagado = 120000.0,
                            usuarioRV_fk = 1,
                            vuelo_fk = 1
                        },
                        new
                        {
                            id = 2,
                            cantPersonas = 1,
                            pagado = 110000.0,
                            usuarioRV_fk = 2,
                            vuelo_fk = 2
                        },
                        new
                        {
                            id = 3,
                            cantPersonas = 1,
                            pagado = 140000.0,
                            usuarioRV_fk = 3,
                            vuelo_fk = 3
                        },
                        new
                        {
                            id = 4,
                            cantPersonas = 1,
                            pagado = 125000.0,
                            usuarioRV_fk = 4,
                            vuelo_fk = 4
                        },
                        new
                        {
                            id = 5,
                            cantPersonas = 2,
                            pagado = 130000.0,
                            usuarioRV_fk = 5,
                            vuelo_fk = 5
                        },
                        new
                        {
                            id = 6,
                            cantPersonas = 2,
                            pagado = 120000.0,
                            usuarioRV_fk = 6,
                            vuelo_fk = 1
                        },
                        new
                        {
                            id = 7,
                            cantPersonas = 2,
                            pagado = 110000.0,
                            usuarioRV_fk = 7,
                            vuelo_fk = 2
                        },
                        new
                        {
                            id = 8,
                            cantPersonas = 1,
                            pagado = 140000.0,
                            usuarioRV_fk = 8,
                            vuelo_fk = 3
                        },
                        new
                        {
                            id = 9,
                            cantPersonas = 2,
                            pagado = 125000.0,
                            usuarioRV_fk = 9,
                            vuelo_fk = 4
                        },
                        new
                        {
                            id = 10,
                            cantPersonas = 2,
                            pagado = 130000.0,
                            usuarioRV_fk = 10,
                            vuelo_fk = 5
                        },
                        new
                        {
                            id = 1002,
                            cantPersonas = 1,
                            pagado = 120000.0,
                            usuarioRV_fk = 3,
                            vuelo_fk = 1
                        },
                        new
                        {
                            id = 2010,
                            cantPersonas = 1,
                            pagado = 125000.0,
                            usuarioRV_fk = 1024,
                            vuelo_fk = 4
                        },
                        new
                        {
                            id = 2011,
                            cantPersonas = 1,
                            pagado = 130000.0,
                            usuarioRV_fk = 1024,
                            vuelo_fk = 5
                        });
                });

            modelBuilder.Entity("Agencia.Models.Usuario", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"), 1L, 1);

                    b.Property<string>("apellido")
                        .IsRequired()
                        .HasColumnType("varchar(50)");

                    b.Property<bool>("bloqueado")
                        .HasColumnType("bit");

                    b.Property<string>("clave")
                        .IsRequired()
                        .HasColumnType("varchar(50)");

                    b.Property<double>("credito")
                        .HasColumnType("float");

                    b.Property<int>("dni")
                        .HasColumnType("int");

                    b.Property<int>("intentosFallidos")
                        .HasColumnType("int");

                    b.Property<bool>("isAdmin")
                        .HasColumnType("bit");

                    b.Property<string>("mail")
                        .IsRequired()
                        .HasColumnType("varchar(255)");

                    b.Property<string>("nombre")
                        .IsRequired()
                        .HasColumnType("varchar(50)");

                    b.HasKey("id");

                    b.ToTable("usuario", (string)null);

                    b.HasData(
                        new
                        {
                            id = 1,
                            apellido = "Pérez",
                            bloqueado = false,
                            clave = "123",
                            credito = 1000000.0,
                            dni = 12345678,
                            intentosFallidos = 0,
                            isAdmin = true,
                            mail = "juan@gmail.com",
                            nombre = "Juan"
                        },
                        new
                        {
                            id = 2,
                            apellido = "Gómez",
                            bloqueado = false,
                            clave = "123",
                            credito = 800000.0,
                            dni = 98765432,
                            intentosFallidos = 0,
                            isAdmin = true,
                            mail = "maria@gmail.com",
                            nombre = "María"
                        },
                        new
                        {
                            id = 3,
                            apellido = "López",
                            bloqueado = false,
                            clave = "123",
                            credito = 500000.0,
                            dni = 45678901,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "carlos@gmail.com",
                            nombre = "Carlos"
                        },
                        new
                        {
                            id = 4,
                            apellido = "Martínez",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 78436532,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "ana@gmail.com",
                            nombre = "Ana"
                        },
                        new
                        {
                            id = 5,
                            apellido = "Rodríguez",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 12344356,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "laura@gmail.com",
                            nombre = "Laura"
                        },
                        new
                        {
                            id = 6,
                            apellido = "Sánchez",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 41234324,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "eduardo@gmail.com",
                            nombre = "Eduardo"
                        },
                        new
                        {
                            id = 7,
                            apellido = "Fernández",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 76474567,
                            intentosFallidos = 0,
                            isAdmin = true,
                            mail = "lucia@gmail.com",
                            nombre = "Lucía"
                        },
                        new
                        {
                            id = 8,
                            apellido = "García",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 98667667,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "javier@gmail.com",
                            nombre = "Javier"
                        },
                        new
                        {
                            id = 9,
                            apellido = "Hernández",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 23455433,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "sofia@gmail.com",
                            nombre = "Sofía"
                        },
                        new
                        {
                            id = 10,
                            apellido = "Pérez",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 74545345,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "carlos2@gmail.com",
                            nombre = "Carlos"
                        },
                        new
                        {
                            id = 11,
                            apellido = "López",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 12313455,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "mariana@gmail.com",
                            nombre = "Mariana"
                        },
                        new
                        {
                            id = 12,
                            apellido = "Martínez",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 53467454,
                            intentosFallidos = 0,
                            isAdmin = true,
                            mail = "andres@gmail.com",
                            nombre = "Andrés"
                        },
                        new
                        {
                            id = 13,
                            apellido = "Gómez",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 23467636,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "valentina@gmail.com",
                            nombre = "Valentina"
                        },
                        new
                        {
                            id = 14,
                            apellido = "Santos",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 43765457,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "diego@gmail.com",
                            nombre = "Diego"
                        },
                        new
                        {
                            id = 15,
                            apellido = "Torres",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 43254756,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "marcela@gmail.com",
                            nombre = "Marcela"
                        },
                        new
                        {
                            id = 16,
                            apellido = "Díaz",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 78787878,
                            intentosFallidos = 0,
                            isAdmin = true,
                            mail = "alejandro@gmail.com",
                            nombre = "Alejandro"
                        },
                        new
                        {
                            id = 17,
                            apellido = "Ramírez",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 12344321,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "luis@gmail.com",
                            nombre = "Luis"
                        },
                        new
                        {
                            id = 18,
                            apellido = "Soto",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 98776655,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "ana2@gmail.com",
                            nombre = "Ana"
                        },
                        new
                        {
                            id = 19,
                            apellido = "Silva",
                            bloqueado = false,
                            clave = "123",
                            credito = 1200000.0,
                            dni = 11223344,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "juan2@gmail.com",
                            nombre = "Juan"
                        },
                        new
                        {
                            id = 1023,
                            apellido = "prueba",
                            bloqueado = false,
                            clave = "123",
                            credito = 1000000.0,
                            dni = 12348765,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "prueba@gmail.com",
                            nombre = "prueba"
                        },
                        new
                        {
                            id = 1024,
                            apellido = "PRUEBA",
                            bloqueado = false,
                            clave = "123",
                            credito = 10000000.0,
                            dni = 123345,
                            intentosFallidos = 0,
                            isAdmin = false,
                            mail = "PRUEBA",
                            nombre = "HOLA"
                        });
                });

            modelBuilder.Entity("Agencia.Models.UsuarioHabitacion", b =>
                {
                    b.Property<int>("usuarios_fk")
                        .HasColumnType("int");

                    b.Property<int>("habitaciones_fk")
                        .HasColumnType("int");

                    b.Property<int>("cantidad")
                        .HasColumnType("int");

                    b.HasKey("usuarios_fk", "habitaciones_fk");

                    b.HasIndex("habitaciones_fk");

                    b.ToTable("usuarioHabitacion");

                    b.HasData(
                        new
                        {
                            usuarios_fk = 1,
                            habitaciones_fk = 1,
                            cantidad = 2
                        },
                        new
                        {
                            usuarios_fk = 2,
                            habitaciones_fk = 7,
                            cantidad = 0
                        },
                        new
                        {
                            usuarios_fk = 3,
                            habitaciones_fk = 1,
                            cantidad = 1
                        },
                        new
                        {
                            usuarios_fk = 3,
                            habitaciones_fk = 13,
                            cantidad = 1
                        },
                        new
                        {
                            usuarios_fk = 4,
                            habitaciones_fk = 19,
                            cantidad = 1
                        },
                        new
                        {
                            usuarios_fk = 5,
                            habitaciones_fk = 25,
                            cantidad = 0
                        },
                        new
                        {
                            usuarios_fk = 6,
                            habitaciones_fk = 1,
                            cantidad = 1
                        },
                        new
                        {
                            usuarios_fk = 7,
                            habitaciones_fk = 7,
                            cantidad = 0
                        },
                        new
                        {
                            usuarios_fk = 8,
                            habitaciones_fk = 13,
                            cantidad = 1
                        },
                        new
                        {
                            usuarios_fk = 9,
                            habitaciones_fk = 19,
                            cantidad = 0
                        },
                        new
                        {
                            usuarios_fk = 10,
                            habitaciones_fk = 25,
                            cantidad = 0
                        },
                        new
                        {
                            usuarios_fk = 18,
                            habitaciones_fk = 1,
                            cantidad = 2
                        },
                        new
                        {
                            usuarios_fk = 1024,
                            habitaciones_fk = 1,
                            cantidad = 1
                        },
                        new
                        {
                            usuarios_fk = 1024,
                            habitaciones_fk = 7,
                            cantidad = 0
                        },
                        new
                        {
                            usuarios_fk = 1024,
                            habitaciones_fk = 25,
                            cantidad = 2
                        },
                        new
                        {
                            usuarios_fk = 1024,
                            habitaciones_fk = 31,
                            cantidad = 0
                        },
                        new
                        {
                            usuarios_fk = 1024,
                            habitaciones_fk = 37,
                            cantidad = 1
                        },
                        new
                        {
                            usuarios_fk = 1024,
                            habitaciones_fk = 121,
                            cantidad = 9
                        });
                });

            modelBuilder.Entity("Agencia.Models.UsuarioVuelo", b =>
                {
                    b.Property<int>("usuario_fk")
                        .HasColumnType("int");

                    b.Property<int>("vuelo_fk")
                        .HasColumnType("int");

                    b.HasKey("usuario_fk", "vuelo_fk");

                    b.HasIndex("vuelo_fk");

                    b.ToTable("usuarioVuelo");

                    b.HasData(
                        new
                        {
                            usuario_fk = 1,
                            vuelo_fk = 1
                        },
                        new
                        {
                            usuario_fk = 2,
                            vuelo_fk = 2
                        },
                        new
                        {
                            usuario_fk = 3,
                            vuelo_fk = 1
                        },
                        new
                        {
                            usuario_fk = 3,
                            vuelo_fk = 3
                        },
                        new
                        {
                            usuario_fk = 4,
                            vuelo_fk = 4
                        },
                        new
                        {
                            usuario_fk = 5,
                            vuelo_fk = 5
                        },
                        new
                        {
                            usuario_fk = 6,
                            vuelo_fk = 1
                        },
                        new
                        {
                            usuario_fk = 7,
                            vuelo_fk = 2
                        },
                        new
                        {
                            usuario_fk = 8,
                            vuelo_fk = 3
                        },
                        new
                        {
                            usuario_fk = 9,
                            vuelo_fk = 4
                        },
                        new
                        {
                            usuario_fk = 10,
                            vuelo_fk = 5
                        },
                        new
                        {
                            usuario_fk = 18,
                            vuelo_fk = 1
                        },
                        new
                        {
                            usuario_fk = 1024,
                            vuelo_fk = 1
                        },
                        new
                        {
                            usuario_fk = 1024,
                            vuelo_fk = 4
                        },
                        new
                        {
                            usuario_fk = 1024,
                            vuelo_fk = 5
                        });
                });

            modelBuilder.Entity("Agencia.Models.Vuelo", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("id"), 1L, 1);

                    b.Property<string>("aerolinea")
                        .IsRequired()
                        .HasColumnType("varchar(50)");

                    b.Property<string>("avion")
                        .IsRequired()
                        .HasColumnType("varchar(50)");

                    b.Property<int>("capacidad")
                        .HasColumnType("int");

                    b.Property<double>("costo")
                        .HasColumnType("float");

                    b.Property<int>("destino_fk")
                        .HasColumnType("int");

                    b.Property<DateTime>("fecha")
                        .HasColumnType("datetime");

                    b.Property<int>("origen_fk")
                        .HasColumnType("int");

                    b.Property<int>("vendido")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.HasIndex("destino_fk");

                    b.HasIndex("origen_fk");

                    b.ToTable("vuelo", (string)null);

                    b.HasData(
                        new
                        {
                            id = 1,
                            aerolinea = "Aerolínea Buenos Aires",
                            avion = "Boeing 747",
                            capacidad = 150,
                            costo = 22000.0,
                            destino_fk = 3,
                            fecha = new DateTime(2023, 10, 1, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 1,
                            vendido = 0
                        },
                        new
                        {
                            id = 2,
                            aerolinea = "Aerolínea Córdoba",
                            avion = "Airbus A320",
                            capacidad = 120,
                            costo = 21000.0,
                            destino_fk = 4,
                            fecha = new DateTime(2023, 10, 2, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 2,
                            vendido = 0
                        },
                        new
                        {
                            id = 3,
                            aerolinea = "Aerolínea Mendoza",
                            avion = "Boeing 787 Dreamliner",
                            capacidad = 180,
                            costo = 24000.0,
                            destino_fk = 5,
                            fecha = new DateTime(2023, 10, 3, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 3,
                            vendido = 0
                        },
                        new
                        {
                            id = 4,
                            aerolinea = "Aerolínea Buenos Aires",
                            avion = "Airbus A320",
                            capacidad = 160,
                            costo = 22500.0,
                            destino_fk = 1,
                            fecha = new DateTime(2023, 10, 4, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 4,
                            vendido = 0
                        },
                        new
                        {
                            id = 5,
                            aerolinea = "Aerolínea Córdoba",
                            avion = "Boeing 747",
                            capacidad = 130,
                            costo = 23000.0,
                            destino_fk = 2,
                            fecha = new DateTime(2023, 10, 5, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 5,
                            vendido = 0
                        },
                        new
                        {
                            id = 6,
                            aerolinea = "Aerolínea Córdoba",
                            avion = "Boeing 787 Dreamliner",
                            capacidad = 170,
                            costo = 23500.0,
                            destino_fk = 3,
                            fecha = new DateTime(2023, 10, 6, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 1,
                            vendido = 0
                        },
                        new
                        {
                            id = 7,
                            aerolinea = "Aerolínea Buenos Aires",
                            avion = "Airbus A320",
                            capacidad = 140,
                            costo = 22800.0,
                            destino_fk = 4,
                            fecha = new DateTime(2023, 10, 7, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 2,
                            vendido = 0
                        },
                        new
                        {
                            id = 8,
                            aerolinea = "Aerolínea Mendoza",
                            avion = "Boeing 747",
                            capacidad = 110,
                            costo = 21500.0,
                            destino_fk = 5,
                            fecha = new DateTime(2023, 10, 8, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 3,
                            vendido = 0
                        },
                        new
                        {
                            id = 9,
                            aerolinea = "Aerolínea Córdoba",
                            avion = "Boeing 787 Dreamliner",
                            capacidad = 190,
                            costo = 24500.0,
                            destino_fk = 1,
                            fecha = new DateTime(2023, 10, 9, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 4,
                            vendido = 0
                        },
                        new
                        {
                            id = 10,
                            aerolinea = "Aerolínea Buenos Aires",
                            avion = "Airbus A320",
                            capacidad = 155,
                            costo = 22700.0,
                            destino_fk = 2,
                            fecha = new DateTime(2023, 10, 10, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 5,
                            vendido = 0
                        },
                        new
                        {
                            id = 11,
                            aerolinea = "Aerolínea Mendoza",
                            avion = "Boeing 747",
                            capacidad = 170,
                            costo = 23500.0,
                            destino_fk = 3,
                            fecha = new DateTime(2023, 10, 11, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 1,
                            vendido = 0
                        },
                        new
                        {
                            id = 12,
                            aerolinea = "Aerolínea Córdoba",
                            avion = "Boeing 787 Dreamliner",
                            capacidad = 120,
                            costo = 21000.0,
                            destino_fk = 4,
                            fecha = new DateTime(2023, 10, 12, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 2,
                            vendido = 0
                        },
                        new
                        {
                            id = 13,
                            aerolinea = "Aerolínea Buenos Aires",
                            avion = "Airbus A320",
                            capacidad = 140,
                            costo = 22800.0,
                            destino_fk = 5,
                            fecha = new DateTime(2023, 10, 13, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 3,
                            vendido = 0
                        },
                        new
                        {
                            id = 14,
                            aerolinea = "Aerolínea Mendoza",
                            avion = "Boeing 747",
                            capacidad = 180,
                            costo = 24000.0,
                            destino_fk = 1,
                            fecha = new DateTime(2023, 10, 14, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 4,
                            vendido = 0
                        },
                        new
                        {
                            id = 15,
                            aerolinea = "Aerolínea Córdoba",
                            avion = "Boeing 787 Dreamliner",
                            capacidad = 130,
                            costo = 23000.0,
                            destino_fk = 2,
                            fecha = new DateTime(2023, 10, 15, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 5,
                            vendido = 0
                        },
                        new
                        {
                            id = 16,
                            aerolinea = "Aerolínea Buenos Aires",
                            avion = "Airbus A320",
                            capacidad = 150,
                            costo = 22000.0,
                            destino_fk = 3,
                            fecha = new DateTime(2023, 10, 16, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 1,
                            vendido = 0
                        },
                        new
                        {
                            id = 17,
                            aerolinea = "Aerolínea Mendoza",
                            avion = "Boeing 747",
                            capacidad = 160,
                            costo = 22500.0,
                            destino_fk = 4,
                            fecha = new DateTime(2023, 10, 17, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 2,
                            vendido = 0
                        },
                        new
                        {
                            id = 18,
                            aerolinea = "Aerolínea Córdoba",
                            avion = "Boeing 787 Dreamliner",
                            capacidad = 190,
                            costo = 24500.0,
                            destino_fk = 5,
                            fecha = new DateTime(2023, 10, 18, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 3,
                            vendido = 0
                        },
                        new
                        {
                            id = 19,
                            aerolinea = "Aerolínea Buenos Aires",
                            avion = "Airbus A320",
                            capacidad = 155,
                            costo = 22700.0,
                            destino_fk = 1,
                            fecha = new DateTime(2023, 10, 19, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 4,
                            vendido = 0
                        },
                        new
                        {
                            id = 20,
                            aerolinea = "Aerolínea Mendoza",
                            avion = "Boeing 747",
                            capacidad = 110,
                            costo = 21500.0,
                            destino_fk = 2,
                            fecha = new DateTime(2023, 10, 20, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 5,
                            vendido = 0
                        },
                        new
                        {
                            id = 22,
                            aerolinea = "PRUEBA",
                            avion = "PRUEBA",
                            capacidad = 110,
                            costo = 21000.0,
                            destino_fk = 19,
                            fecha = new DateTime(2023, 10, 25, 9, 0, 0, 0, DateTimeKind.Unspecified),
                            origen_fk = 1,
                            vendido = 0
                        });
                });

            modelBuilder.Entity("Agencia.Models.Habitacion", b =>
                {
                    b.HasOne("Agencia.Models.Hotel", "hotel")
                        .WithMany("habitaciones")
                        .HasForeignKey("hotel_fk")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("hotel_fk");

                    b.Navigation("hotel");
                });

            modelBuilder.Entity("Agencia.Models.Hotel", b =>
                {
                    b.HasOne("Agencia.Models.Ciudad", "ubicacion")
                        .WithMany("hoteles")
                        .HasForeignKey("ciudad_fk")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("ciudad_fk");

                    b.Navigation("ubicacion");
                });

            modelBuilder.Entity("Agencia.Models.ReservaHabitacion", b =>
                {
                    b.HasOne("Agencia.Models.Habitacion", "miHabitacion")
                        .WithMany("misReservas")
                        .HasForeignKey("habitacion_fk")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("habitacion_fk");

                    b.HasOne("Agencia.Models.Usuario", "miUsuario")
                        .WithMany("misReservasHabitaciones")
                        .HasForeignKey("usuarioRH_fk")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("usuarioRH_fk");

                    b.Navigation("miHabitacion");

                    b.Navigation("miUsuario");
                });

            modelBuilder.Entity("Agencia.Models.ReservaVuelo", b =>
                {
                    b.HasOne("Agencia.Models.Usuario", "miUsuario")
                        .WithMany("misReservasVuelos")
                        .HasForeignKey("usuarioRV_fk")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("usuarioRV_fk");

                    b.HasOne("Agencia.Models.Vuelo", "miVuelo")
                        .WithMany("misReservas")
                        .HasForeignKey("vuelo_fk")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("vuelo_fk");

                    b.Navigation("miUsuario");

                    b.Navigation("miVuelo");
                });

            modelBuilder.Entity("Agencia.Models.UsuarioHabitacion", b =>
                {
                    b.HasOne("Agencia.Models.Habitacion", "habitacion")
                        .WithMany("habitacion_usuario")
                        .HasForeignKey("habitaciones_fk")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Agencia.Models.Usuario", "usuario")
                        .WithMany("usuario_habitacion")
                        .HasForeignKey("usuarios_fk")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("habitacion");

                    b.Navigation("usuario");
                });

            modelBuilder.Entity("Agencia.Models.UsuarioVuelo", b =>
                {
                    b.HasOne("Agencia.Models.Usuario", "usuario")
                        .WithMany("usuario_vuelo")
                        .HasForeignKey("usuario_fk")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Agencia.Models.Vuelo", "vuelo")
                        .WithMany("vuelo_usuario")
                        .HasForeignKey("vuelo_fk")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("usuario");

                    b.Navigation("vuelo");
                });

            modelBuilder.Entity("Agencia.Models.Vuelo", b =>
                {
                    b.HasOne("Agencia.Models.Ciudad", "destino")
                        .WithMany()
                        .HasForeignKey("destino_fk")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired()
                        .HasConstraintName("destino_fk");

                    b.HasOne("Agencia.Models.Ciudad", "origen")
                        .WithMany("vuelos")
                        .HasForeignKey("origen_fk")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired()
                        .HasConstraintName("origen_fk");

                    b.Navigation("destino");

                    b.Navigation("origen");
                });

            modelBuilder.Entity("Agencia.Models.Ciudad", b =>
                {
                    b.Navigation("hoteles");

                    b.Navigation("vuelos");
                });

            modelBuilder.Entity("Agencia.Models.Habitacion", b =>
                {
                    b.Navigation("habitacion_usuario");

                    b.Navigation("misReservas");
                });

            modelBuilder.Entity("Agencia.Models.Hotel", b =>
                {
                    b.Navigation("habitaciones");
                });

            modelBuilder.Entity("Agencia.Models.Usuario", b =>
                {
                    b.Navigation("misReservasHabitaciones");

                    b.Navigation("misReservasVuelos");

                    b.Navigation("usuario_habitacion");

                    b.Navigation("usuario_vuelo");
                });

            modelBuilder.Entity("Agencia.Models.Vuelo", b =>
                {
                    b.Navigation("misReservas");

                    b.Navigation("vuelo_usuario");
                });
#pragma warning restore 612, 618
        }
    }
}
